...
public:
    ...
    template<typename KeyType, typename ValType> 
    bool find(const KeyType &key, vector<ValType> &val)
    {
        string tmp_key;
        tmp_key = m_name + bitTostring(key);    
        ...
        {
           stringTobit(res[index], tmp_vals[index]); 
        }
        val.swap(res);
        return true;
    }
    ...
    template<typename KeyType, typename ValType>
    int count(const KeyType &key, const ValType &val)
    {
        string tmp_key;
        tmp_key = m_name + bitTostring(key);      
        string tmp_val;
    ...
    {            
        return basic_redis::count(m_name + bitTostring(key), m_db);     
    }
    
    template<typename KeyType, typename ValType>    
    bool erase(const KeyType &key, const ValType &val)
    {
        string tmp_key;
        tmp_key = m_name + bitTostring(key);      
        string tmp_val;
