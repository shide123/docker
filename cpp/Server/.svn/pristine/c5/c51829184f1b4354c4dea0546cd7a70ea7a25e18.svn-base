...
public:
    ...
    bool redis_scan_keys(const string &filter, vector<string> &val, int db = 0);
	bool redis_keys(const string &filter, vector<string> &val, int db = 0);
    int count(const string &key, int db);
	bool expire(const string &key, unsigned int live, int db);
	bool expireat(const string &key, unsigned int expiration, int db);
	bool del(const string &key, int db);
protected:
